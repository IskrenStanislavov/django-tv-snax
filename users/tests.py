...
class UserTest(TestCase):
    ...
    def create_user(self):
        ...
        return get_user_model().objects.create_user(self.user_email, self.user_password, **self.extra_fields)

    def test_user_creation(self):
        # Create a new user saving the time frame
        before_creation = timezone.now() - timezone.timedelta(10)
        self.create_user()
        after_creation = timezone.now() + timezone.timedelta(10)

        # Check user exists and email is correct
        self.assertEqual(get_user_model().objects.all().count(), 1)
        self.assertEqual(get_user_model().objects.all()[0].email, self.user_email)

        self.assertFalse( hasattr(get_user_model().objects.all()[0],"user_id"))
        self.assertLess(0, get_user_model().objects.all()[0].id)
        self.assertLess(0, get_user_model().objects.all()[0].pk)
        self.assertEqual(0, get_user_model().objects.all()[0].points)

        # self.assertEqual(self.extra_fields.get("username", ""), get_user_model().objects.all()[0].username)
        self.assertEqual(self.extra_fields.get("address", ""), get_user_model().objects.all()[0].address)
        for (key,value) in self.extra_fields.items():
            self.assertEqual(value, getattr(get_user_model().objects.all()[0], key))

        # Check date_joined date
    ...
    def test_givePoints(self):
        user = self.create_user()
        user.givePoints(100);
        self.assertEqual(user.points, 100)
        user.givePoints(700);
        self.assertEqual(user.points, 800)

        
    def test_checkPrizeAvailableTrue(self):
        user = self.create_user()
        user.givePoints(100);
        self.assertTrue(user.checkPrizeAvailable(10))
        
    def test_checkPrizeAvailableFalse(self):
        user = self.create_user()
        user.givePoints(100);
        self.assertFalse(user.checkPrizeAvailable(1000))
        
    def test_takePointsSuccess(self):
        user = self.create_user()
        user.givePoints(100);
        self.assertEqual(user.takePoints(10), 90)

    def test_takePointsMoreThanUserHas(self):
        user = self.create_user()
        user.givePoints(100);
        self.assertRaises(ValueError, user.takePoints, 1000)
...
class UserManagerTest(TestCase):
    ...
    def test_create_user(self):
        ...
        self.assertFalse(hasattr(user, "has_usable_password"))
    ...
    def test_create_superuser(self):
        ...
        self.assertTrue(user.is_active)
        self.assertFalse(user.is_staff, "We do not use a SuperUser")
        self.assertFalse(user.is_superuser, "We do not use a SupoerUser")
    ...
    def test_user_creation_is_staff(self):
        ...
        email_lowercase = 'normal@normal.com'
        password = 'password1234$%&/'
        user = get_user_model().objects.create_user(email_lowercase, password)
        self.assertFalse(user.is_staff)
...
class EmailUserChangeFormTest(TestCase):
    ...
    def test_bug_19133(self):
        ...
        post_data.update(UserTest.extra_fields)
...
@skipIf('django.contrib.admin' not in settings.INSTALLED_APPS, "Django Admin not installed")
class EmailUserAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        if settings.AUTH_USER_MODEL == "custom_user.User":
